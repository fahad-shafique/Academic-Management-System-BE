...
def club_list(request) -> Response:
    ...
    page = paginator.paginate_queryset(clubs_query, request)
    if page is not None:
        serializer = s.ClubListSerializer(page, many=True)
        return paginator.get_paginated_response(serializer.data)
    else:
        serializer = s.ClubListSerializer(clubs_query, many=True)
...
def event_list(request) -> Response:
    ...
    paginator = PageNumberPagination()
    page = paginator.paginate_queryset(events_query, request)
    if page is not None:
        serializer = s.ClubListSerializer(page, many=True)
        return paginator.get_paginated_response(serializer.data)
    else:
        serializer = s.ClubListSerializer(events_query, many=True)
        return Response(serializer.data)
